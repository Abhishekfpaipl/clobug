<template>
    <div style="padding-top:80px">
        ...
        <div class=" d-md-flex d-block container mb-4 w-100">
            ...
            <div class="w-100">
                <div v-for="(product, index) in filteredAndSortedOrder" :key="index" class="border p-2 mb-2">
...
export default {
    ...
    computed: {
        ...mapState({
            order: state => state.order.order, // Replace yourModuleName with your actual module name
        }),
        filteredAndSortedOrder() {
            const currentDate = new Date();
            const selectedRange = parseInt(this.selectedDateRange);

            const filteredOrder = this.order.filter(item => {
                const itemDate = new Date(item.date);
                const daysDifference = Math.floor((currentDate - itemDate) / (1000 * 60 * 60 * 24));
                return daysDifference <= selectedRange;
            });
